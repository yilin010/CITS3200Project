...
function updateRow(row, i, reset) {
  ...
  function generateMarkers(sNumber){
      ...
      for (var i = 0; i < checks-1; i++) {
          if(document.getElementById(i).getElementsByTagName('td')[0].innerText == sNumber ){
                ...
                if (html.length > 0){
                    ...
                    for(var j=0;j<rows;j++){
                        ...
                        for(var i=0;i<columns;i++){
                            subTable += "<td class\"classRow\" onclick=\"generateMarkers("+data[j*columns]+");return false;\">"+data[i+j*columns]+"</td>";
                        }
                        subTable +="</tr>";
                    }
